...
export async function initRoutes(app: Router, joi: any, network: Network) {
    ...
    app.get('/api/teams/:key/warpgroups', async function (req, res) {
        ...
        const value = await buildTeam.getWarpGroups();

        if(value == null) {
            res.status(400).send({ error: 'Build Team Warp Group for this Build Team not found.' });
            return;
        }
