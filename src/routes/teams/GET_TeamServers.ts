...
export async function initRoutes(app: Router, joi: any, network: Network) {
    ...
    app.get('/api/teams/:key/servers', async function (req, res) {
        ...
        const value = await buildTeam.getBuildTeamInfo("Servers");

        if(value == null) {
            res.status(400).send({ error: 'Build Team Servers for this Build Team not found' });
            return;
        }
