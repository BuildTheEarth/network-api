...
export async function initRoutes(app: Router, joi: any, network: Network) {
    ...
    app.get('/api/teams/:key/continent', async function (req, res) {
        ...
        const value = await buildTeam.getBuildTeamInfo("Continent");

        if(value == null) {
            res.status(400).send({ error: 'Build Team Continent for this Build Team not found' });
            return;
        }
        
        res.setHeader('Content-Type', 'application/json');
        res.send(JSON.stringify({buildTeamContinent : value}))
    })
}